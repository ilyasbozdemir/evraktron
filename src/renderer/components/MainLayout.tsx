...
export function MainLayout() {
  ...
  useEffect(() => {
    window.evraktron.appVersion().then(setAppVersion).catch(console.error);
  }, []);
        ...
        >
          ...
          <div className="p-3 border-t border-surface-700 relative">
            <button 
              ...
              className="flex items-center gap-2 w-full p-2 rounded-lg transition-colors text-xs text-surface-400 hover:text-surface-200 hover:bg-surface-800/50"
            >
              <div className="w-6 h-6 rounded-md bg-brand-500/20 text-brand-400 flex items-center justify-center shrink-0">
                <Info className="w-3.5 h-3.5" />
              </div>
              <div className="flex-1 text-left">
                <p className="font-medium text-surface-300">Evraktron v{appVersion}</p>
                <p className="text-[10px] opacity-70">Hakkında & Güncelleme</p>
              </div>
            </button>
          </div>
        </aside>
